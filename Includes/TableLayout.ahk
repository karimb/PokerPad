 ...
 *
 *  Optional Parameters:
 *  	xi: the x position from the windows top left corner to position the table
 *  		if a blank variable is passed, the x AND y position will be that of the first control in
 *  		the parameter ctrls, both xi and yi will be updated with the x,y position of the table
 *  	yi: the y position from the windows top left corner to position the table
 *  	totWidth:
 *  		width of the table including margins and spacing
 *  		contents will be ignored if resizeRules is not specified
 *  		the resulting width of the table is stored in this variable regardless
 *  	totHeight:
 *  		height of the table including margins and spacing
 *  		contents will be ignored if resizeRules is not specified
 *  		the resulting height of the table is stored in this variable regardless
 ...
 *  		number of controls to put in cell
 *  		note that they will overlap (useful for hiding/showing different controls)
 *  		If 0 is specified, the cell will be left empty.
 *  	r# [rowSpan] default = 1
 *  		rows the control spans
 *  	c# [colSpan] default = 1
 *  		columns the control spans
 *  	a# [align] default = 0
 *  		alignment of control in cell
 *  		0: left
 *  		1: center
 *  		2: right
 *  	v# [vAlign] default = 0
 *  		vertical alignment of control in cell
 *  		0: top
 *  		1: middle
 *  		2: bottom
 *  	h# [height] default = 0
 *  		minimum height of cell
 *  		If this is set (>0), the control will be expanded to fit the height of its cell (minus padding)
 ...
 *  		See Also f#
 *  	f# [fixed control size] default = 0
 *  		0: not fixed, control will expand to the width of its cell if w# is specified and the height of its cell if h# is specified
 *  		1: fixed width
 *  		2: fixed height
 *  		3: fixed width and height
 *  	p# [padding] default = 0
 *  		width and height of the padding between the cell edges and control edges
 ...
 *
 *  Example 1: Laying out all controls in a window.
 *  	Gui, +LastFound
 ...
 *  	Gui, Add, Edit
 *  	Gui, Add, CheckBox
 *  	Size := TableLayout_Window("a2 v1||v1", 5, 5, 5)
 *  		; a2 - right alignment
 *  		; v1 - middle alignment
 *  	Gui, Show, %Size%
 *
 *  Example 2: Laying out all controls in a resizable window.
 *  	Gui, +LastFound +Resize
 *  	Gui, Margin , 0, 0
 *  	Gui, Add, Edit ; all edit controls are used in the example to give each cell a visual representation.
 *  	Gui, Add, Edit
 *  	Gui, Add, Edit
 *  	Gui, Add, Edit
 *  	Gui, Add, Edit
 *  	Gui, Add, Edit
 *  	Size := TableLayout_WindowResize("w100 h20|w100 h1|w100 h1 r2_w1 h20 c2_w1 c3_w1 h20 c3,5,5,5,r3")
 *  		; comma delimited list of rules, spacing, marginX, marginY, resizeRuiles
 *  		; r3 in resizeRules will fix the height of row 3
 *  	Gui, Show, %Size%
 *  	return
 *  	GuiSize:
 *  		TableLayout_WindowResize()
 *  		return
 */
 
TableLayout_Window(rules, spacing, marginX, marginY, ByRef ctrls = "", ByRef x = "", ByRef y = "", ByRef w = "", ByRef h = "") {
	if !ctrls
